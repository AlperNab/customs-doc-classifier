...
### Operations UX for purchase, supplier, warehouse, customs, and document reconciliation workflows.
...
**Domain:** `Supply Chain / Customs` · **Suite:** `Supply Chain Suite` · **Accent:** `#fdba74`
...
**Customs Doc Classifier** is a standalone, browser-based AI workflow app for **Supply Chain / Customs**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
**UX profile:** `Supply Chain Operations Desk`

**Workflow layout:** Document/email intake → matching → discrepancy/risk → action/export

**Empty state:** Upload supply-chain documents or paste supplier emails. Customs/live carrier data requires real connectors.
...
|---|---:|:---:|---|---|
| `shipping_docs`<br>Shipping docs | text | Yes | — | Affects input: Shipping docs. |
...
| `origin_destination`<br>origin/destination | text | No | — | Affects customization: origin/destination. |
| `incoterms`<br>Incoterms | text | No | — | Affects customization: Incoterms. |
| `currency`<br>currency | select | No | USD | Affects customization: currency. |
| `product_category`<br>product category | text | No | — | Affects customization: product category. |
| `risk_strictness`<br>risk strictness | slider | No | 50 | Affects customization: risk strictness. |
| `preferred_hs_level`<br>preferred HS level | select | No | — | Affects customization: preferred HS level. |
| `broker_notes`<br>broker notes | textarea | No | — | Affects customization: broker notes. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9113
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9113`. |
...
- Shipping docs
- HS code and duty/compliance review
...
- Export download
- Job history
- Customs/tariff database required for final HS/duty
...
- supplier/ERP/WMS connector when syncing operational records
- official tariff/customs source when classifying documents
- approval gate for compliance-sensitive actions
...
```bash
curl http://127.0.0.1:9113/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9113/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9113 uvicorn app.main:app --host 0.0.0.0 --port 9113
```
...
- `plugins/customs-doc-classifier.json` for plugin metadata.
